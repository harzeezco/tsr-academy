...
export function Card({ course }: { course: CourseDataProp }) {
  ...
  return (
    <Link href={href}>
      <li
        key={title}
        className='relative rounded-2xl transition-all after:rounded-2xl  after:bg-[rgba(0,0,0,0.3)] after:transition-all after:content-[""] after:hover:absolute after:hover:inset-y-0 after:hover:left-0 after:hover:size-full active:scale-95 after:active:absolute'
      >
        <Image
          alt={alt}
          className='w-full'
